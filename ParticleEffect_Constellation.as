package
{
    
    import flash.display.Bitmap;
    import flash.display.Sprite;
    import flash.events.Event;
    import flash.events.MouseEvent;
    import flash.media.Sound;
    
  	import fl.transitions.Tween;
	import fl.transitions.TweenEvent;
	import fl.transitions.easing.*;
      
    public class ParticleEffect_Constellation extends ParticleEffect
    {
        var last_ms_played:int=0;
		var b:Bitmap;
		var particles:Array;
		var maxparticles:int;
        var c:Object;
		
        public function ParticleEffect_Constellation(bit:Bitmap,cont:Object)
        {
		c=cont;
		b=bit;
		particles=new Array;
		maxparticles=0;
        }
        
        public function advancemilliseconds(ms:int)
		{
		var totalms:int;
		totalms=last_ms_played+ms;
		
		//1 sec per particle
		var i:int;
		
		//init them
		for(i=0;i<120;i++)
		{
		//if(totalms>i*40 && last_ms_played<i*40+1)
		if(last_ms_played==0)
		{
		particles[maxparticles]=new ParticleXA(b,100,100);
		c.addChild(particles[maxparticles]);
		var angle2:Number;
		angle2=30*i;
					trace("Anglea"+angle2);

		angle2=angle2/10;
					trace("Angleb"+angle2);

			
			angle2=angle2*(2*Math.PI) / 360;
var dx:Number;
var dy:Number;

//angle2=Math.PI/2;
//trace("ANGLE"+angle2);
					dx=Math.cos(angle2);
					trace("dx"+dx);
				
				var dz:int;
				dz=getrandomint(0,50);
				
				dx=((100+dz)*dx);
				dy=((100+dz)*(Math.sin(angle2)));
				
				///trace("dy"+dy);
					dx=dx+300;
					dy=dy+300;

		particles[maxparticles].angle=angle2;


		particles[maxparticles].x=dx;
		particles[maxparticles].y=dy;
		
		particles[maxparticles].scaleX=.1;
		particles[maxparticles].scaleY=.1;
		
		
		particles[maxparticles].tweena = new Tween(particles[maxparticles].b,"scaleX",None.easeInOut,1,8,20);
		particles[maxparticles].tweenb = new Tween(particles[maxparticles].b,"scaleY",None.easeInOut,1,8,20);
		
			particles[maxparticles].tweena.addEventListener(TweenEvent.MOTION_FINISH, onTweenFinish1);


		
		
		
		maxparticles++;
		
		}
		
		
		}
		
		last_ms_played=totalms;
		return false;		
		}
 
 
 
 		function onTweenFinish1(e:TweenEvent):void {
			var i:int;
			var a:int=-1;
			for(i=0;i<maxparticles;i++)
				if(Tween(e.target) == particles[i].tweena)
				{
					a=i;
					i=maxparticles;
				}
			
			Tween(e.target).removeEventListener(TweenEvent.MOTION_FINISH, onTweenFinish1);
var dx:Number;
var dy:Number;

					dx=Math.cos(particles[a].angle);
				dx=((45+(a%2)*6)*dx);
				dy=((45+(a%2)*6)*(Math.sin(particles[a].angle)));
			trace("Angle"+	particles[a].angle);
					dx=dx+300;
					dy=dy+300;

			//alltweens[a].tween  = new Tween(alltweens[a].image,"alpha",Strong.easeOut,1,0,1,40);
			//alltweens[a].tween.addEventListener(TweenEvent.MOTION_FINISH, onTween2Finish);
			if(a==119)
			next1();
			
			//alltweens[a].tween2  = new Tween(alltweens[a].image,"y",None.easeInOut,alltweens[a].ggy*60-40-(alltweens[a].gridy+1)*60+alltweens[a].incy,alltweens[a].ggy*60-120-(alltweens[a].gridy+1)*60+alltweens[a].incy,40);
			
		}


private function next1()
{
trace("finished");
var i:int;
for(i=0;i<120;i++)
{
var a:int=i;
var dx:Number;
var dy:Number;

					dx=Math.cos(particles[i].angle);
				dx=((5)*dx);
				dy=((5)*(Math.sin(particles[i].angle)));
			trace("Angle"+	particles[i].angle);
					dx=dx+300;
					dy=dy+300;


			particles[a].tweena = new Tween(particles[a],"x",None.easeInOut,particles[a].x,dx,45);
			particles[a].tweenb = new Tween(particles[a],"y",None.easeInOut,particles[a].y,dy,45);

if(a==0)
			particles[a].tweena.addEventListener(TweenEvent.MOTION_FINISH, onTweenFinish2);


if(a%2)
{
particles[a].tweenc = new Tween(particles[a].b,"scaleX",Regular.easeIn,8,16,40);
		particles[a].tweend = new Tween(particles[a].b,"scaleY",Regular.easeIn,8,16,40);
}
else
{
particles[a].tweenc = new Tween(particles[a].b,"scaleX",None.easeInOut,8,3,40);
		particles[a].tweend = new Tween(particles[a].b,"scaleY",None.easeInOut,8,3,40);
}		


}


}
 
 
 function onTweenFinish2(e:TweenEvent):void {
			var i:int;
			var a:int=-1;
			for(i=0;i<maxparticles;i++)
				if(Tween(e.target) == particles[i].tweena)
				{
					a=i;
					i=maxparticles;
				}
			
			Tween(e.target).removeEventListener(TweenEvent.MOTION_FINISH, onTweenFinish2);

var a:int;
for(a=0;a<maxparticles;a++)
{
var dx:Number;
var dy:Number;

					dx=Math.cos(particles[a].angle);
				

if(a%2)
{				
				dx=((200)*dx);
				dy=((200)*(Math.sin(particles[a].angle)));
}
else
{
				dx=((400)*dx);
				dy=((400)*(Math.sin(particles[a].angle)));

}			
			
					dx=dx+300;
					dy=dy+300;

			particles[a].tweena = new Tween(particles[a],"x",Strong.easeOut,particles[a].x,dx,20);
			particles[a].tweenb = new Tween(particles[a],"y",Strong.easeOut,particles[a].y,dy,20);

}
			//alltweens[a].tween  = new Tween(alltweens[a].image,"alpha",Strong.easeOut,1,0,1,40);
			//alltweens[a].tween.addEventListener(TweenEvent.MOTION_FINISH, onTween2Finish);
			
			//alltweens[a].tween2  = new Tween(alltweens[a].image,"y",None.easeInOut,alltweens[a].ggy*60-40-(alltweens[a].gridy+1)*60+alltweens[a].incy,alltweens[a].ggy*60-120-(alltweens[a].gridy+1)*60+alltweens[a].incy,40);
			
		}

 
 		private function getrandomint(low:int,high:int)
		{
			return Math.floor(Math.random()*(1+high-low))+low;
		}

        
    }

    
}